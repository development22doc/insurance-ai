...
export const AdminLayout: React.FC = () => {
  ...
  const navigation = [
    ...
    { name: 'AI Configuration', href: '/admin/ai' },
    { name: 'Fraud/Risk', href: '/admin/risk' },
          ...
          <nav className="flex-1 p-4 space-y-1">
            {navigation.map((item) => (
              <Link
                key={item.name}
                to={item.href}
                className="block px-4 py-2 text-sm font-medium text-[var(--color-text-secondary)] hover:text-[var(--color-warning)] hover:bg-[var(--color-surface)] rounded-lg transition-colors"
                onClick={() => setSidebarOpen(false)}
              >
                {item.name}
              </Link>
