...
class ClaimSagaOrchestratorServiceTest {
    ...
    void duplicateOrchestrationRequestIsIgnored() {
        ...
        verify(sagaOutboxPublisher, never()).enqueueIfAbsent(anyString(), anyString(), anyString(), anyString(), anyString());
    }
    ...
    void terminalSagaSkipsDispatch() {
        ...
        verify(sagaOutboxPublisher, never())
                .enqueueIfAbsent(anyString(), anyString(), eq(ClaimSagaOrchestratorService.STEP_COMMAND_TOPIC), anyString(), anyString());
    }
    ...
    void successfulStepAdvancesSaga() {
        ...
        orchestrator.handleStepResult(new ClaimSagaStepResultEvent("r1", "s1", SagaActionType.CREATE_CLAIM,
                SagaStepType.CREATE_CLAIM, true, 100L, null, 1));
    ...
    void paymentFailureTriggersCompensation() {
        ...
        orchestrator.handleStepResult(new ClaimSagaStepResultEvent("r1", "s1", SagaActionType.CREATE_CLAIM,
                SagaStepType.PAYMENT, false, 100L, "payment declined", 1));
    ...
    @Test
    void notCompletedOnNullStepResultIsNotRelevantButFailurePublishesFinalResult() {
        ...
        orchestrator.handleStepResult(new ClaimSagaStepResultEvent("r1", "s1", SagaActionType.CREATE_CLAIM,
                SagaStepType.NOTIFICATION, false, 100L, "notif failed", 1));

        // NOTIFICATION failure is NOT terminal: it flows to COMPENSATION
        assertThat(saga.getStatus()).isEqualTo(SagaOrchestrationStatus.COMPENSATING);
    }
