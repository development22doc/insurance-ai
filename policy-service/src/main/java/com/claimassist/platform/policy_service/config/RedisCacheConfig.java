...
public class RedisCacheConfig {
    ...
    public static final String PLANS_BY_PRODUCT_CACHE = "plansByProduct";
    public static final String POLICY_COVERAGE_CACHE = "policyCoverage";

    /** PRODUCT_CACHE TTL: product catalog reference data changes infrequently (admin/operations only). */
    private static final Duration PRODUCT_TTL = Duration.ofMinutes(15);
    /** PLAN_CACHE TTL: plan catalog reference data changes infrequently (admin/operations only). */
    private static final Duration PLAN_TTL = Duration.ofMinutes(15);
    /** PLANS_BY_PRODUCT_CACHE TTL: same as plan cache - plans by product is equally stable. */
    private static final Duration PLANS_BY_PRODUCT_TTL = Duration.ofMinutes(15);
    /** POLICY_COVERAGE_CACHE TTL: hottest read path (called by Claims/Agent on every claim check);
     * correctness-sensitive enough that staleness must stay bounded. Matches customer-service. */
    ...
    public RedisTemplate<String, Object> redisTemplate(
        ...
        objectMapper = objectMapper.copy()
                .activateDefaultTyping(
                    ...
                    // Use EVERYTHING (not NON_FINAL) so final record DTOs (ProductDto, PlanDto,
                    // PolicyCoverageDto) get @class type metadata. NON_FINAL omits type info for
                    // final types, so records could never be deserialized back out of Redis.
                    // EVERYTHING is the same default Spring Data Redis's own
                    // GenericJackson2JsonRedisSerializer uses.
    ...
    public RedisCacheManagerBuilderCustomizer cacheManagerCustomizer() {
        ...
        objectMapper = objectMapper.copy()
                .activateDefaultTyping(
                    objectMapper.getPolymorphicTypeValidator(),
                    // EVERYTHING typing so final records round-trip (see redisTemplate bean above).
                    ObjectMapper.DefaultTyping.EVERYTHING
                );
        ...
        return builder -> builder
            ...
            .withCacheConfiguration(
                PRODUCT_CACHE,
                defaultConfig.entryTtl(PRODUCT_TTL))
            .withCacheConfiguration(
                PLAN_CACHE,
                defaultConfig.entryTtl(PLAN_TTL))
            .withCacheConfiguration(
                PLANS_BY_PRODUCT_CACHE,
                defaultConfig.entryTtl(PLANS_BY_PRODUCT_TTL))
            .withCacheConfiguration(
